...
@OnlyIn(Dist.CLIENT)
public class PMM2_BipedArmorLayer<T extends Entity, M extends PMM2_BipedModel<T>, A extends PMM2_BipedModel<T>> extends LayerRenderer<T, M> {
   ...
   public PMM2_BipedArmorLayer(IEntityRenderer<T, M> entityRendererIn, A modelLeggings, A modelArmor) {
   ...
   protected void setModelSlotVisible(A model, EquipmentSlotType slotIn) {
      ...
      case CHEST:
         ...
         model.bipedArmorRightArm.showModel = true;
         model.bipedArmorLeftArm.showModel = true;
         model.bipedUpperBoob.showModel = true;
         model.bipedLowerBoob.showModel = true;
         break;
      case LEGS:
         model.bipedBody.showModel = true;
         model.bipedArmorRightLeg.showModel = true;
         model.bipedArmorLeftLeg.showModel = true;
         break;
      case FEET:
         model.bipedArmorRightLeg.showModel = true;
         model.bipedArmorLeftLeg.showModel = true;
   ...
   private void renderArmorLayer(T entityLivingBaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale, EquipmentSlotType slotIn) {
      ItemStack itemstack = ((LivingEntity) entityLivingBaseIn).getItemStackFromSlot(slotIn);
      if (itemstack.getItem() instanceof ArmorItem) {
         ArmorItem armoritem = (ArmorItem)itemstack.getItem();
         if (armoritem.getEquipmentSlot() == slotIn) {
            A a = this.func_215337_a(slotIn);
            a.entityIn = entityLivingBaseIn;
            a.doWalkBounding = false;
            a = getArmorModelHook((LivingEntity) entityLivingBaseIn, itemstack, slotIn, a);
